...
CREATE TABLE accounts (
                          ...
                          username VARCHAR(50) NOT NULL UNIQUE,    -- 用户名唯一，不可重复
                          phone VARCHAR(20) UNIQUE,                -- 手机号唯一，用于登录
...
-- study_tour 标签字典初始化脚本
-- 表：tags(name, type)
-- type 枚举：INTEREST / ROUTE_STYLE / CROWD / SCENIC
...
ALTER TABLE account_tag_prefs DROP COLUMN weight;
ALTER TABLE route_tags DROP COLUMN weight;
ALTER TABLE leader_profiles DROP COLUMN years_of_exp;
ALTER TABLE leader_profiles DROP COLUMN base_region_code;
